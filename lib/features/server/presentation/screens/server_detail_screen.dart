...
class ServerDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final stateAsync = ref.watch(serverDetailControllerProvider(serverId));
        ...
        actions: [
          if (stateAsync.value?.server != null)
            PopupMenuButton<String>(
              enabled: !(stateAsync.value?.isDeleting ?? false),
      ...
      body: stateAsync.when(
        data: (state) {
          if (state.server == null) {
            return const ErrorText(message: 'Server not found');
          }
